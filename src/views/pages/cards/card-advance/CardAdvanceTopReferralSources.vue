...
<template>
  <VCard title="Top Referral Sources" subtitle="82% activity growth">
    <template #append>
      ...
      <VSlideGroup v-model="currentTab" show-arrows mandatory>
        ...
        >
          <div
            ...
            class="!flex flex-col justify-center align-center cursor-pointer rounded px-5 py-2 me-4"
            style="width: 7rem; height: 6rem"
            @click="toggle"
          >
            <VImg v-bind="category.img" alt="slide-img" />
          </div>
        </VSlideGroupItem>

        <VSlideGroupItem>
          <div
            class="!flex flex-col justify-center align-center rounded me-4 cursor-pointer not-selected-category"
            style="width: 7rem; height: 6rem"
          >
            <VAvatar rounded size="30" color="default" variant="tonal">
              <VIcon icon="mdi-plus" />
            </VAvatar>
          </div>
        </VSlideGroupItem>
    ...
    <VTable class="text-no-wrap mb-2">
      ...
      <tbody class="text-high-emphasis">
        ...
        >
          <td style="width: 6vw">
            <VAvatar rounded :image="currentProduct.image" size="34" />
          </td>

          <td class="font-weight-semibold" style="width: 20vw">
          ...
          <td class="font-weight-semibold text-end">
            <span
              :class="
                currentProduct.conversion > 0 ? 'text-success' : 'text-error'
              "
            >
              {{
                currentProduct.conversion > 0
                  ? `+${currentProduct.conversion}%`
                  : `${currentProduct.conversion}%`
              }}
            </span>
          </td>
